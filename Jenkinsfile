pipeline {
	...
	stages {
		...
		stage ("push docker image") {
			...
			echo "tagging image"
			sh 'sudo docker tag jotonia:1.2 jakeni/jotoniaapp1:v5'
			withCredentials([usernamePassword(credentialsId: 'DockerHub', passwordVariable: 'pass', usernameVariable: 'user')]) {
  			echo "pushing container to hub"
			sh 'sudo docker login -u jakeni -p $pass'
  			sh 'sudo docker push jakeni/jotoniaapp1:v5'
		...
		stage ("deploy docker image") {
			...
			echo "Deployning latest version"
                        sh 'ssh jakeni@192.168.33.12 sudo docker run -d --rm -p 80:80 jakeni/jotoniaapp1:v5 '
                }
        }
